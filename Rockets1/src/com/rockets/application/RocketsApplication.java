...
public class RocketsApplication {
	...
	public void CreateRockets(String rocketid, int boosternumber) throws Exception {
		try {
			Rocket rocket= new Rocket(rocketid,boosternumber);
			rocket.AddBoosters(CreateBooster(boosternumber));//añadimos 
	...
	public List<Booster> CreateBooster(int boosternumber){
		List<Booster> boostersinrocket = new ArrayList<Booster>();
		for(int i=0; i <boosternumber;i++) {
			boostersinrocket.add(new Booster());
		}
